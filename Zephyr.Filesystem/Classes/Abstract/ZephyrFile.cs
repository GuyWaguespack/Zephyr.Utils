...
namespace Zephyr.Filesystem
{
    ...
    public abstract class ZephyrFile
    {
        ...
        public abstract bool Delete(bool stopOnError = true, bool verbose = true, String callbackLabel = null, Action<string, string> callback = null);
        ...
        public void CopyTo(ZephyrFile file, bool overwrite = true, bool createMissingDirectories = true, bool stopOnError = true, bool verbose = true, String callbackLabel = null, Action<string, string> callback = null)
        {
            try
            {
                ...
                // Clear Out Destination File If It Already Exists
                if (file.Exists)
                {
                    file.Delete(stopOnError, verbose, callbackLabel, callback);
                    file.Create(overwrite, verbose, callbackLabel, callback);
                }

                Stream source = this.Open(AccessType.Read, verbose);
                Stream target = file.Open(AccessType.Write, verbose);

                // TODO : If Target File Bigger Than Source File, Doesn't Wipe Extra Data From Target File
